...
3. **The front door is drawn even if you haven't built it.** Most estates start with no
   classifier — every request straight onto the big iron. Draw the router anyway, greyed if
...
- **Draw what runs, not what you intend.** If your "review gate" is a sentence in an instruction
  file, it goes on the *policy* plane — putting it in enforcement would be exactly the
...
- **Expect three to five findings.** Typical harvest from a first honest L2: no front door (all
  ...
  evals. Each finding maps to a lesson and to a design-brief section (lesson 11) — your L2 *is*
...
> Segmentation went behavioral, because data and control share one channel now — a hostile
...
*Done-tests: L1 stands alone — a reader who sees only that diagram gets the
five planes, the security model, and the front-door finding. L2 is drawn from the learner's
reality — the skeleton enforces it by being useless until filled. L3 compresses the module into
