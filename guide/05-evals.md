...
routing lessons, because the classifier's eval (lesson 10) leans on this one.*
...
estate, producing a score you can track over time. That's it: a synthetic-transaction
monitor for behavior. Building one has the same anatomy as building a good check ever did:

1. **Collect real cases.** A dozen to a few hundred actual inputs from your estate's real
   traffic: real requests, real documents, real edge cases. Synthetic-only test sets measure your
...
definition of what "working" means. If you can't write the eval, if you can't produce ten real
inputs and say what good output looks like, you don't yet know what you're building, and the
model will resolve that ambiguity for you (lesson 4 told you how it resolves ambiguity).
...
> itself, and if we haven't defined 'working,' we can't see 'broken.'"
