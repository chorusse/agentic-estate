...
and tools one-liners and the code-dispatch row (rows 5, 16, 17, 22). No diagram — the artifact of
this lesson is a table, and the table is the point.*
...
a summary route, the classic fat-finger `/8` — doesn't just fail; it *attracts* traffic that
...
Recall the code-dispatch row (row 22): when routing is code — a classifier returns a label and a
`switch` picks the branch — the table is explicit and auditable. But much of an agentic estate
...
The model reads the request, reads all the advertisements currently in scope, and matches — its
...
advertisement in the estate — every trigger phrase, description field, tool doc — into one page,
one row each: **traffic class → advertisement text → handler → tier/budget.** Then read it as a
routing table, asking the table questions: Which classes have no row? (Unannounced prefixes —
...
**Teach:** skill triggers, agent descriptions, and tool docs ARE your routing table — writing
descriptions is writing route entries. **Bite:** an over-broad advertisement attracts traffic
that belonged elsewhere and nothing drops — the gaps only become visible when you design the
table *as* a table.
