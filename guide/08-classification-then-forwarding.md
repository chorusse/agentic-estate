...
lesson absorbs classification economics — the cost model is half of the same idea.*
...
thought of "deciding where traffic goes" as expensive — reading the header is free by
construction.
...
traffic *doesn't say* where it's going — something has to read the content and infer.
...
**on the wire, classification is free — read the header at wire speed. Here, classification costs
...
(lessons 9 and 10 both build on it). Most people find their taxonomy is small — four to seven
classes cover ninety-plus percent — and that a majority of traffic is cheap lookups currently
...
**Classification-then-forwarding — teach:** a packet carries its destination; a prompt doesn't —
