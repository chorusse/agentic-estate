...
the lawful-intercept infrastructure, the places where operators control the network — because
whoever holds the management plane holds everything, silently, with legitimate-looking hands.
Every hardened estate you've run internalized this: the jump boxes, the config repositories, the
firmware chain, the admin credentials — that layer outranks the whole data plane, because the
data plane *obeys* it.

So: locate the management plane of an agentic estate. It's not the model — you can't reconfigure
the model. It's the layer that controls what the model does, every session, deterministically or
near-deterministically. Look at what that actually is:
...
- **Hold every change for human review.** Any diff touching a management-plane path is held
  before it propagates — a deterministic gate (lesson 2: enforcement plane, not a polite request)
...
That closes the trust arc. Next: the routing arc — starting with the one difference between your
