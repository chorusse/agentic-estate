...
one-liners and lesson 9 (route advertisements). No diagram — the concept is contractual, not
spatial.*
...
3. **Interfaces drift without changing.** The same instruction file behaves differently under a
   ...
   *Response:* version instruction files like code — reviewed diffs, history, rollback — and
...
interface — and the phrase generalizes: you are writing interfaces for a reader that infers, so
...
> "Every prompt and charter is an interface consumed by a reader that infers — there are no
...
write it as a contract for a smart stranger. **Bite:** your ini files were never "interpreted
