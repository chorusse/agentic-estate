...
5. **Capacity plan → tier and budget map** *(lesson 8's economics + the tiers row).* Each task
   ...
   table from lesson 9 — advertisements, handlers, and the lesson-10 default route as its last
...
7. **Change control → egress and review gates** *(lessons 6 and 7).* Everything that leaves the
   estate — pushes, sends, deploys, publishes — and the deterministic gate each one passes.
...
The fix isn't ceremony; it's sequencing. The brief doesn't need to be long — a page per section
...
> document before; only the material is new. And it has to lead the build — an as-built diagram
